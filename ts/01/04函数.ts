...
let result1_1 = buildName1('Bob'); // works correctly now
// let result2_1 = buildName1("Bob", "Adams", "Sr.");  // error, too many parameters
let result3_1 = buildName1('Bob', 'Adams'); // ah, just right
...
let result1_2 = buildName2('Bob'); // works correctly now, returns "Bob Smith"
...
let result4_2 = buildName2('Bob', 'Adams'); // ah, just right
...
interface Deck2 {
    suits: string[];
    cards: number[];
    createCardPicker(this: Deck2): () => Card2;
}
let deck2: Deck2 = {
    suits: ["hearts", "spades", "clubs", "diamonds"],
    cards: Array(52),
    // NOTE: The function now explicitly specifies that its callee must be of type Deck
    createCardPicker: function(this: Deck2) {
        return () => {
            let pickedCard = Math.floor(Math.random() * 52);
            let pickedSuit = Math.floor(pickedCard / 13);
            console.log('pickedSuit=',pickedSuit)  
...
function pickCard3(x: number):void {  };
function pickCard4(x:any): any {
    // Check to see if we're working with an object/array
    // if so, they gave us the deck and we'll pick the card
    if (typeof x == "object") {
        let pickedCard = Math.floor(Math.random() * x.length);
...
let myDeck4 = [{ suit: "diamonds", card: 2 }, { suit: "spades", card: 10 }, { suit: "hearts", card: 4 }];
